...
if __name__ == '__main__':
    ...
    print("garment_org_body_unposed.v.shape : ")
    print(garment_org_body_unposed.v.shape)
    print("garment_org_body_unposed.f.shape : ")
    print(garment_org_body_unposed.f.shape)

    garment_org_body_unposed = Mesh(garment_org_body_unposed.v, garment_org_body_unposed.f)
    mvs = MeshViewers((1, 2), keepalive=True)
    mvs[0][0].set_static_meshes([garment_org_body_unposed])
    mvs[0][1].set_static_meshes([garment_org_body_unposed])
    sleep(30)
    ...
    #mvs = MeshViewers((1, 2))  # <= the original line that was written by BLB, Mon Mar  2 23:35:29 EST 2020.  -nxb
    # but I'm pretty sure with "\`keepalive=True\`," this will not be an issue.
    print("\n"*9)
    print("tgt_body : ")
    print(type(tgt_body))
    print(tgt_body.v.shape)
    print("type(garment_unposed) : ")
    print(type(garment_unposed)  )
    print("garment_unposed.v.shape : ")
    print(garment_unposed.v.shape)
    print("new_garment.v.shape : ")
    print(new_garment.v.shape)
    print("\n"*9)
    mvs = MeshViewers((1, 2), keepalive=True)
